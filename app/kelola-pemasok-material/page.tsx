// @/app/kelola-pemasok-material/page.tsx
import Sidebar from "@/components/Sidebar";
import { getSuppliers } from "@/lib/actions/supplier";
import { redirect } from "next/navigation";
import { Building2, Mail, Phone, MapPin, Package, Plus } from "lucide-react";
import Link from "next/link";

export default async function KelolaPemasokMaterialPage() {
	let suppliers;
			...
			<main className="flex-1 lg:ml-64 p-4 lg:p-8">
				...
				<div className="flex justify-between items-center mb-6">
					<div>
						<h1 className="text-3xl font-bold text-foreground">
							Kelola Pemasok Material
						</h1>
						<p className="text-gray-600 mt-1">
							Manajemen data pemasok dan supplier material
						</p>
					</div>
					<Link
						href="/kelola-pemasok-material/create"
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
					<div className="bg-white p-5 rounded-3xl">
						<p className="text-gray-600 text-sm">Total Pemasok</p>
						<p className="text-2xl font-bold text-foreground">
							{suppliers.length}
						</p>
					</div>
					<div className="bg-white p-5 rounded-3xl">
						<p className="text-gray-600 text-sm">Pemasok Aktif</p>
						<p className="text-2xl font-bold text-green-600">
							{suppliers.filter((s) => s.isActive).length}
						</p>
					</div>
					<div className="bg-white p-5 rounded-3xl">
						<p className="text-gray-600 text-sm">Pemasok Nonaktif</p>
						...
						>
							...
							<div className="space-y-3 mb-4">
								...
								<div className="flex items-start gap-2 text-sm text-gray-600">
									...
									<span className="line-clamp-2">{supplier.alamatPerusahaan}</span>
								</div>
							</div>
							...
							<div className="pt-4 border-t border-gray-200">
								...
								<div className="flex flex-wrap gap-2">
									{supplier.materialYangDiSupply.map((material, idx) => (
										<span
											key={idx}
							...
							<div className="mt-4 pt-4 border-t border-gray-200 flex gap-2">
								<Link
									href={`/kelola-pemasok-material/${supplier._id}`}
				...
				{suppliers.length === 0 && (
					<div className="text-center py-12 bg-white rounded-xl shadow-md">
						...
						<p className="text-gray-500 mb-6">
							Tambahkan pemasok material pertama Anda
						</p>
						<Link
							href="/kelola-pemasok-material/create"
